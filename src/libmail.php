 ...
 * Class SendMail
 *
 * Расширяет Mail чтоб можно было легко слать письма
 *
 *
    Пример:
...
class SendMail extends Mail {
    ...
    protected $keys = []; // Названия полей, в т.ч. файлов (Поля, которых здесь нет, не будут отправлены)
    protected $form = ''; // От кого
    protected $to = ''; // Кому, можно несколько через запятую
    protected $subject = ''; // Тема письма
    protected $validation = []; // Обязательные поля. Если обязательно одно из нескольких полей - указываем их через разделитель "||"


    public function __construct($keys, $validation = [], $charset = "", $ctencoding = '')
    {
        parent::__construct($charset, $ctencoding);
     ...
     * @param $rules - правила
     * @return array|bool - true - если все нормально иначе массив с ошибками
     */
    protected function validate($data = []) {
        ...
        foreach ($this->validation as $rule) {
            ...
            // Если одиночное правили
            if (count($rule) == 1) {
                if ($this->is_empty($data, $rule[0])) {
                    $errors[$rule[0]] = 'Поле ' . $this->get_key($rule[0]) . ' обязательно для заполнения';
                }
            } else {
                // Если правило или то или другое
                $err = false;
                foreach ($rule as $r) {
                    if ($this->is_empty($data, $r)) {
                        $err = true;
                    } else {
                        $err = false;
                        break;
                ...
                if ($err) {
                    $err_key = implode('___', $rule);
                    $errors[$err_key] = 'Одно из полей: ';

                    for ($i = 0, $count = count($rule), $last = $count - 1; $i < $count; $i++) {

                        if ($i == $last) {
                            $errors[$err_key] .= ' или ' . $this->get_key($rule[$i]);
                        } elseif ($i == 0) {
                            $errors[$err_key] .= $this->get_key($rule[$i]);
                        } else {
                            $errors[$err_key] .=  ', ' . $this->get_key($rule[$i]);
                        }
                    }

                    $errors[$err_key] .= ' - должно быть обязательно заполнено.';
                }
            }
        }
    ...
    protected function is_empty($data, $key) {
        ...
        if (isset($var['tmp_name'])) {
            $file = is_array($var['tmp_name']) ? array_filter($var['tmp_name'], '_empty') : $var['tmp_name'];
            return empty($file);
        } elseif (empty(array_filter($var, '_empty'))) { // Это не файл
            return true;
        }
    ...
    /**
     * Возвращает файлы для прикрепления к письму
     * @param $files
     * @return array
     */
    protected function get_files($files = []) {
        $files = $files ?: $_FILES;
    ...
    protected function get_message($data = []) {
        $data = $data ?: $_POST;
        $message = '';
        foreach ($data as $key => $value) {
            if (!isset($this->keys[$key])) continue; // Если поля нет у нас в разрешенных - пропускаем

            $message .= '<p><strong>' . $this->get_key($key) . '</strong>: ' . $this->clear($value) . '</p>';
        }
    ...
    public static function send_mail($from, $to, $subject, $keys, $validation) {
        $to = array_map('trim', explode(',', $to));

        $mail = new SendMail($keys, $validation);


        // Валидация
        if (($result = $mail->validate()) !== true) {
            return $result;
        }
        ...
        $mail->Body($mail->get_message(), 'html');

        if ($files = $mail->get_files()) {
            foreach ($files as $file) {
                $mail->Attach($file['file'], $file['name']);
            }
        }
...
function _empty($var) {
    return (is_string($var) && strlen($var)) || $var ? true : false;
}
